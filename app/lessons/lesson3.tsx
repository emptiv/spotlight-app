  ...
  const steps = [
    {
      ...
      render: () => (
        <View>
          <Text style={styles.title}>Let's try writing ᜑ!</Text>
          <HandwritingCanvas
            key={`canvas-${step}`}
            lesson="lesson3"
            onPrediction={(result) => {
              setPrediction(result);
              setIsCorrect(result.toLowerCase() === "ha");
            }}
            onClear={() => {
              setPrediction(null);
              setIsCorrect(false);
    ...
    },
    {
      ...
      render: () => (
        <View>
          <Text style={styles.title}>Let's try writing ᜊ!</Text>
          <HandwritingCanvas
            key={`canvas-${step}`}
            lesson="lesson3"
            onPrediction={(result) => {
              setPrediction(result);
              setIsCorrect(result.toLowerCase() === "ba");
            }}
            onClear={() => {
              setPrediction(null);
              setIsCorrect(false);
    ...
    },
    {
      ...
      render: () => (
        <View>
          <Text style={styles.title}>Let's try writing ᜄ!</Text>
          <HandwritingCanvas
            key={`canvas-${step}`}
            lesson="lesson3"
            onPrediction={(result) => {
              setPrediction(result);
              setIsCorrect(result.toLowerCase() === "ga");
            }}
            onClear={() => {
              setPrediction(null);
              setIsCorrect(false);
